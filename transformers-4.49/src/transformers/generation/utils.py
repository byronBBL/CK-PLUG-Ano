...
class GenerationMixin:
    ...
    def generate(
        ...
        **kwargs,
        ...
        generation_config_student = copy.deepcopy(generation_config)
        model_kwargs_student = copy.deepcopy(model_kwargs)
        self._validate_model_kwargs(model_kwargs.copy())
        self._validate_model_kwargs(model_kwargs_student.copy())
        ...
        logits_processor_student = logits_processor_student if logits_processor_student is not None else LogitsProcessorList()
        stopping_criteria = stopping_criteria if stopping_criteria is not None else StoppingCriteriaList()

        accepts_attention_mask = "attention_mask" in set(inspect.signature(self.forward).parameters.keys())
        requires_attention_mask = "encoder_outputs" not in model_kwargs
        requires_attention_mask_student = "encoder_outputs" not in model_kwargs_student
        kwargs_has_attention_mask = model_kwargs.get("attention_mask", None) is not None
        kwargs_has_attention_mask_student = model_kwargs_student.get("attention_mask", None) is not None

        # 3. Define model inputs
        inputs_tensor, model_input_name, model_kwargs = self._prepare_model_inputs(
            inputs, generation_config.bos_token_id, model_kwargs
        )
        inputs_tensor_student, model_input_name_student, model_kwargs_student = self._prepare_model_inputs(
            inputs_student, generation_config_student.bos_token_id, model_kwargs_student
        )
        batch_size = inputs_tensor.shape[0]
        batch_size_student = inputs_tensor_student.shape[0]

        device = inputs_tensor.device
        self._prepare_special_tokens(generation_config, kwargs_has_attention_mask, device=device)
        self._prepare_special_tokens(generation_config_student, kwargs_has_attention_mask_student, device=device)
        ...
        if not self.config.is_encoder_decoder and not is_torchdynamo_compiling():
            ...
            if (
                generation_config_student._pad_token_tensor is not None
                and batch_size_student > 1
                and len(inputs_tensor_student.shape) == 2
                and torch.sum(inputs_tensor_student[:, -1] == generation_config_student._pad_token_tensor) > 0
            ):
                logger.warning(
                    "A decoder-only architecture is being used, but right-padding was detected! For correct "
                    "generation results, please set `padding_side='left'` when initializing the tokenizer."
        ...
        if not kwargs_has_attention_mask_student and requires_attention_mask_student and accepts_attention_mask:
            model_kwargs_student["attention_mask"] = self._prepare_attention_mask_for_generation(
                inputs_tensor_student, generation_config_student, model_kwargs_student
            )
        elif kwargs_has_attention_mask:
            # TODO (joao): generalize this check with other types of inputs
            if model_input_name == "input_ids" and len(model_kwargs["attention_mask"].shape) > 2:
        ...
        if self.config.is_encoder_decoder and "encoder_outputs" not in model_kwargs:
            ...
            model_kwargs_student = self._prepare_encoder_decoder_kwargs_for_generation(
                inputs_tensor_student, model_kwargs_student, model_input_name, generation_config_student
            )
        ...
        if self.config.is_encoder_decoder:
            ...
            input_ids_student = inputs_tensor_student if model_input_name == "input_ids" else model_kwargs_student.pop("input_ids")

        if generation_config.token_healing:
            input_ids = self.heal_tokens(input_ids, tokenizer)

        if generation_config_student.token_healing:
            input_ids_student = self.heal_tokens(input_ids_student, tokenizer)
        ...
        input_ids_length_student = input_ids_student.shape[-1]
        ...
        generation_config_student = self._prepare_generated_length(
            generation_config=generation_config_student,
            has_default_max_length=has_default_max_length,
            has_default_min_length=has_default_min_length,
            model_input_name=model_input_name,
            inputs_tensor=inputs_tensor_student,
            input_ids_length=input_ids_length_student,
        )
        ...
        if ck_decoding:
            generation_mode = GenerationMode.CK_GENERATION
        ...
        prepared_logits_processor_student = self._get_logits_processor(
            generation_config=generation_config_student,
            input_ids_seq_length=input_ids_length,
            encoder_input_ids=inputs_tensor_student,
            prefix_allowed_tokens_fn=prefix_allowed_tokens_fn,
            logits_processor=logits_processor_student,
            device=inputs_tensor.device,
            model_kwargs=model_kwargs_student,
            negative_prompt_ids=negative_prompt_ids,
            negative_prompt_attention_mask=negative_prompt_attention_mask,
        )
        ...
        model_kwargs['attention_mask_student'] = model_kwargs_student['attention_mask']
        ...
        elif generation_mode == GenerationMode.CK_GENERATION:
            if self._is_stateful:
                # DoLa decoding was not designed for stateful models, and would require some changes
                raise ValueError(
                    f"CK-PLUG is not supported with stateful models, such as {self.__class__.__name__}"
                )
    ...
    def relative_top_filter(self, scores: torch.FloatTensor, scores_student: torch.FloatTensor, relative_top: float = 0.1, filter_value: float = -1e10, min_tokens_to_keep: int = 1) -> torch.FloatTensor:
        scores_normalized = scores.log_softmax(dim=-1)
        scores_student_normalized = scores_student.log_softmax(dim=-1)
        sorted_logits, sorted_indices = torch.sort(scores_normalized, descending=True)
        sorted_logits_student, sorted_indices_student = torch.sort(scores_student_normalized, descending=True)
        ...
        min_thresh_student = sorted_logits_student[..., min_tokens_to_keep-1]
        probs_max_student = torch.max(scores_student_normalized, dim=-1).values
        probs_thresh_student = probs_max_student + np.log(relative_top)
        probs_thresh_student = torch.min(min_thresh_student, probs_thresh_student)
        probs_thresh_student = probs_thresh_student.unsqueeze(-1)

        mask = (scores_normalized < probs_thresh) & (scores_student_normalized < probs_thresh_student)
        scores[mask] = filter_value
        scores_student[mask] = filter_value
    ...
    def _ck_decoding(
        ...
        **model_kwargs,
        ...
        batch_size_student, cur_len_student = input_ids_student.shape
        this_peer_finished = False
        unfinished_sequences = torch.ones(batch_size, dtype=torch.long, device=input_ids.device)
        unfinished_sequences_student = torch.ones(batch_size_student, dtype=torch.long, device=input_ids_student.device)
        model_kwargs = self._get_initial_cache_position(input_ids, model_kwargs)
        model_kwargs_student = self._get_initial_cache_position(input_ids_student, model_kwargs_student)
        ...
        while self._has_unfinished_sequences(
            ...
            model_inputs_student = self.prepare_inputs_for_generation(input_ids_student, **model_kwargs_student)

            # prepare variable output controls (note: some models won't accept all output controls)
            model_inputs.update({"output_attentions": output_attentions} if output_attentions else {})
            model_inputs.update({"output_hidden_states": output_hidden_states} if output_hidden_states else {})
            model_inputs_student.update({"output_attentions": output_attentions} if output_attentions else {})
            model_inputs_student.update({"output_hidden_states": output_hidden_states} if output_hidden_states else {})

            if is_prefill:
                outputs = self(**model_inputs, return_dict=True)
                outputs_student = self(**model_inputs_student, return_dict=True)
                is_prefill = False
            else:
                outputs = model_forward(**model_inputs, return_dict=True)
                outputs_student = model_forward(**model_inputs_student, return_dict=True)
            ...
            next_token_logits_student = outputs_student.logits[:, -1, :].clone().float()
            next_token_logits_student = next_token_logits_student.to(input_ids_student.device)

            next_token_logits, next_token_logits_student, mask = self.relative_top_filter(next_token_logits, next_token_logits_student, relative_top = 0.01, min_tokens_to_keep=select_top)
            ...
            if is_adjust:
                ...
                next_token_logits_student[mask] = -1e10
